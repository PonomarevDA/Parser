...
## Введение
Есть некоторая известная формула с несколькими переменными. Подстановкой значений этих переменных в данную формулу можно однозначно получить ответ (число).
Введем 2 понятия:
...
**1 Метод перебора [DoReverseCalculateWithBruteForce()](https://github.com/PonomarevDA/Parser/blob/858cb1bda432cfe6ebd3256cf419c99c45a24666/obd.cpp#L281-L335)**
...
**2 Метод половинного деления [DoReverseCalculateWithMethodDichotomy()](https://github.com/PonomarevDA/Parser/blob/858cb1bda432cfe6ebd3256cf419c99c45a24666/obd.cpp#L338-L427)**
...
**3 Метод решения обратной задачи с помощью дерева [DoReverseCalculateWithTree()](https://github.com/PonomarevDA/Parser/blob/858cb1bda432cfe6ebd3256cf419c99c45a24666/obd.cpp#L187-L278)**
...
- Возникают ошибки с некоторыми операторами в формулах, в которых операндами оператора являются другие операторы.

## Описание особенностей реализации функции DoReverseCalculateWithTree()

Для выполнения данного метода, необходимо сначала построить дерево, соответствующее формуле c помощью функции- [CreateTree()](https://github.com/PonomarevDA/Parser/blob/858cb1bda432cfe6ebd3256cf419c99c45a24666/obd.cpp#L15-L74)

Решение обратной задачи вынесено в отдельный класс - Calculator, который является дружественным для OBD. Таким образом осуществляется
разделения протокола OBD и алгоритма расчета.
...
Элементарный обратный расчет реализует функция [CalculateReverseElementary](https://github.com/PonomarevDA/Parser/blob/858cb1bda432cfe6ebd3256cf419c99c45a24666/obd.cpp#L475-L578)
